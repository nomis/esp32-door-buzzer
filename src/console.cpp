...
static inline void setup_commands(std::shared_ptr<Commands> &commands) {
	auto buzzer_debounce_time = [] (Shell &shell,
			const std::vector<std::string> &arguments __attribute__((unused))) {
		Config config;
		unsigned long value = config.buzzer_debounce_time_ms();

		shell.printfln(F("Buzzer debounce time: %lums"), value);
	};
	...
	commands->add_command(ShellContext::MAIN, CommandFlags::USER, CommandFlags::ADMIN,
		flash_string_vector{F("buzzer"), F("debounce"), F("time")},
		buzzer_debounce_time);
	...
	commands->add_command(ShellContext::MAIN, CommandFlags::ADMIN,
		...
		if (!arguments.empty()) {
			...
			config.buzzer_debounce_time_ms(value);
			config.commit();
		}

		buzzer_debounce_time(shell, NO_ARGUMENTS);
	});
